(function () {
	...
	Viewport.prototype.update = function (
		...
		this.startTile[0] =
			tile[0] - 1 - Math.ceil(this.screen[0] / 2 / blockWidth);
		this.startTile[1] =
			tile[1] - 1 - Math.ceil(this.screen[1] / 2 / blockHeight);
		...
		if (
			this.endTile[0] >= this.worldSize[0] &&
			this.worldSize[0] * blockWidth -
				(this.worldSize[0] * blockWidth - playerX) >=
				this.worldSize[0] * blockWidth - this.screen[0] / 2
		) {
			this.offset[0] = -(this.worldSize[0] * blockWidth) + this.screen[0];
			this.startTile[0] = Math.floor(
				this.endTile[0] - this.screen[0] / blockWidth
			);
		}
		...
		if (
			this.endTile[1] >= this.worldSize[1] &&
			this.worldSize[1] * blockHeight -
				(this.worldSize[1] * blockHeight - playerY) >=
				this.worldSize[1] * blockHeight - this.screen[1] / 2
		) {
			this.offset[1] = -(this.worldSize[1] * blockHeight) + this.screen[1];
			this.startTile[1] = Math.floor(
				this.endTile[1] - this.screen[1] / blockHeight
			);
		} //pizdec...
	};
